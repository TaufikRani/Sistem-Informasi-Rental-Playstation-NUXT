<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold">Tarif Main</h1>
        <p class="text-neutral-500 text-sm">Kelola tarif main per jenis room</p>
      </div>
      <UButton icon="i-lucide-plus" @click="openModal()">Tambah Tarif</UButton>
    </div>

    <UCard>
      <UTable :data="items" :loading="loading" :columns="columns" empty="Belum ada tarif">
...
const columns = [
  { id: 'name', key: 'name', label: 'Nama Tarif' },
  { id: 'roomType', key: 'roomType', label: 'Jenis Room' },
  { id: 'hourlyRate', key: 'hourlyRate', label: 'Harga/Jam' },
...
async function onDelete(row: any) {
  if (!confirm(`Hapus tarif "${row.original.name}"?`)) return
