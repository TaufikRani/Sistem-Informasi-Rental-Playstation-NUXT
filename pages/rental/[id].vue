<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold">Rental {{ data?.invoiceNumber }}</h1>
        <p class="text-neutral-500 text-sm">{{ data?.customerName }} {{ data?.customerPhone }}</p>
      </div>
      <div class="flex items-center gap-2">
        <UBadge :color="statusColor(data?.status)" variant="subtle">{{ TRANSACTION_STATUS_LABEL[data?.status] }}</UBadge>
        <UButton v-if="data?.status === 'waiting_return'" color="neutral" variant="outline" icon="i-lucide-x" @click="onCancel">Batalkan</UButton>
        <UButton v-if="data?.status === 'waiting_return'" color="success" icon="i-lucide-check" @click="returnOpen = true">Pengembalian</UButton>
      </div>
    </div>
        ...
        <UCard>
          ...
          <div class="space-y-2 text-sm">
            <div v-for="item in data?.items || []" :key="item.id" class="flex justify-between">
              <span class="text-neutral-500">{{ item.itemName }}</span>
    ...
    <UModal v-model:open="returnOpen">
      <UCard :ui="{ body: 'space-y-4' }">
        ...
        <div v-if="isLate" class="rounded-lg bg-error/5 border border-error/20 p-3 text-sm">
          <div class="font-semibold text-error mb-1">Terlambat {{ estimatedLateHours }} jam</div>
          <div class="text-neutral-600 dark:text-neutral-300">Denda: {{ formatRupiah(estimatedLateHours * hourlyPenalty) }}</div>
        </div>
        <div v-else class="text-sm text-neutral-500">Tidak ada keterlambatan.</div>
...
async function onDeleteItem(row: any) {
  ...
  await $fetch(`/api/transactions/rental/${id}/items/${row.original.id}`, { method: 'DELETE' })
  await refresh()
  await refreshProducts()
}
...
async function onCancel() {
  if (!confirm('Batalkan transaksi rental ini? Perangkat akan dikembalikan ke status ready.')) return
